...
While many of them implicitly or explicitly rely on the correctness and adequacy proof as spelled out by Launchbury, some stick with the original definition of the heap semantics using $\sqcup$, for which the proofs do not got through, while others use right-sided updates, without further explanation. The work by Baker-Finch {\em et~al.} is particularly interesting, as they switched from the original to the fixed definition between an earlier tech report \shortcite{parallel-tr} and the later ICFP publication \shortcite{parallel}, unfortunately without motivating that change. Nakata also changes this detail between the two cited works.
...
In particular S{\'a}nchez-Gil {\em et~al.} worked towards rigorously proving Launchbury's semantics correct and adequate. They noted that the relation between the standard and the resourced denotational semantics is not as trivial as it seemed at first, and and worked out a detailed pen-and-paper proof \shortcite{functionspaces}. We have formalized this, fixing mistakes in the proof, and build on their result here (\myref{Lemma}{lem_denrel}).
...
We can help ourselves here by letting our computers check the proofs. This is what we have done: We took an established formalism and with the help of the theorem prover Isabelle, weeded out all mistakes, filled all the holes and clarified a lot of technical details.
