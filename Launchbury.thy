...
case (Application y \<Gamma> e xa L \<Delta> \<Theta> z e' L')
  ...
  proof(rule reds.Application)
    ...
    hence "x \<notin> heapVars \<Delta>"
      by (metis set_mp reds_doesnt_forget[OF Application.hyps(11)])

    have "atom x \<sharp> (\<Gamma>, e)"
      using `atom x \<sharp> (\<Gamma>, App e xa, \<Theta>, z)`
      by (simp add: fresh_Pair exp_assn.fresh)
    from reds_fresh[OF Application.hyps(9) this] `x \<notin> heapVars \<Delta>`
    ...
    ultimately
    show "\<Gamma> : e \<Down>\<^bsub>xa # L'\<^esub> \<Delta> : Lam [y]. e'"
    ...
    from reds_fresh[OF Application.hyps(11) this] `x \<notin> heapVars \<Theta>`
    ...
    show "\<Delta> : e'[y::=xa] \<Down>\<^bsub>L'\<^esub> \<Theta> : z "
      by (rule Application.hyps(12)[OF _ `set L' = _`])
  qed
next 
