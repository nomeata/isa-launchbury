...
\item A generalisation of some parts of HOLCF to work on a set as a carrier instead of a type (\isa{HOLCF-Set}, \isa{HOLCF-Set-Nominal}).
...
\item The theory \isa{Fmap-Utils} adds lemmas that combine finite maps with the various other non-standard auxillary theories.
\end{itemize}
...
Chapter \ref{ch_denupd} defines the denotational semantics where $\sqcup$ means right-sided update, and proves Launchbury's semantics correct using the proof from his paper. Again, the theory \isa{HSem} is abstract in the concrete denotation of expressions.
