...
| "set (bn as) \<sharp>* \<rho> \<Longrightarrow> \<lbrakk> Let as body \<rbrakk>\<^bsub>\<rho>\<^esub> = \<lbrakk>body\<rbrakk>\<^bsub>has_ESem.UHSem ESem (asToHeap as) \<rho>\<^esub>"
proof-
...
case (goal13 as \<rho> body as' \<rho>' body')
  assume eqvt1: "\<And> e x. e \<in> snd ` set (asToHeap as) \<Longrightarrow> eqvt_at ESem_sumC (e, x)"
    and eqvt2:"eqvt_at ESem_sumC (body, has_ESem.UHSem (\<lambda>x0 x1. ESem_sumC (x0, x1)) (asToHeap as) \<rho>)"
    and eqvt3:"\<And>e x. e \<in> snd ` set (asToHeap as') \<Longrightarrow> eqvt_at ESem_sumC (e, x)"
    and eqvt4:"eqvt_at ESem_sumC (body', has_ESem.UHSem (\<lambda>x0 x1. ESem_sumC (x0, x1)) (asToHeap as') \<rho>')"
  ...
  have "ESem_sumC (body, has_ESem.UHSem (\<lambda>x0 x1. ESem_sumC (x0, x1)) (asToHeap as) \<rho>) =
        ESem_sumC (body', has_ESem.UHSem (\<lambda>x0 x1. ESem_sumC (x0, x1)) (asToHeap as') \<rho>)"
  proof (rule Abs_lst_fcb[where ba = bn and f = "(\<lambda> as (body, as'). ESem_sumC (body, has_ESem.UHSem (\<lambda>x0 x1. ESem_sumC (x0, x1)) (asToHeap as) \<rho>))", OF tmp pure_fresh pure_fresh, simplified])
    fix p
    ...
    have "(p \<bullet> ESem_sumC) (body', has_ESem.UHSem (\<lambda>x xa. (p \<bullet> ESem_sumC) (x, xa)) (asToHeap as') (p \<bullet> \<rho>)) =
          (p \<bullet> ESem_sumC) (body', has_ESem.UHSem (\<lambda>x xa. ESem_sumC (x, xa)) (asToHeap as') (p \<bullet> \<rho>))"
      apply (rule arg_cong[OF UHSem_cong[OF _ refl refl]])
      apply (subst eqvt_at_apply[OF eqvt3], assumption)
      by rule
    also have "\<dots> = (p \<bullet> ESem_sumC) (body', has_ESem.UHSem (\<lambda>x xa. ESem_sumC (x, xa)) (asToHeap as') \<rho>')"
      by (simp add: `p \<bullet> \<rho> = \<rho>` `\<rho>' = \<rho>`)
    also have "\<dots> = ESem_sumC (body', has_ESem.UHSem (\<lambda>x xa. ESem_sumC (x, xa)) (asToHeap as') \<rho>')"
      by (rule eqvt_at_apply[OF eqvt4])
    also have "\<dots> = ESem_sumC (body', has_ESem.UHSem (\<lambda>x xa. ESem_sumC (x, xa)) (asToHeap as') \<rho>)"
      by (simp add: `\<rho>' = \<rho>`)
    finally
    show "(p \<bullet> ESem_sumC) (body', has_ESem.UHSem (\<lambda>x xa. (p \<bullet> ESem_sumC) (x, xa)) (asToHeap as') (p \<bullet> \<rho>)) =
               ESem_sumC (body', has_ESem.UHSem (\<lambda>x0 x1. ESem_sumC (x0, x1)) (asToHeap as') \<rho>)".
  qed
  thus "ESem_sumC (body, has_ESem.UHSem (\<lambda>x0 x1. ESem_sumC (x0, x1)) (asToHeap as) \<rho>) =
      ESem_sumC (body', has_ESem.UHSem (\<lambda>x0 x1. ESem_sumC (x0, x1)) (asToHeap as') \<rho>')" using `\<rho>' = \<rho>`  by simp
qed auto
...
lemmas UHSem_eqvt' = UHSem_eqvt[of _ ESem, unfolded permute_ESem]

lemmas UHSem_fresh[simp] = eqvt_fresh_cong2[of UHSem, OF UHSem_eqvt']
 and   UHSem_fresh_star[simp] = eqvt_fresh_star_cong2[of UHSem, OF UHSem_eqvt']
