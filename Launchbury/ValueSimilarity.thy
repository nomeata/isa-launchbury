...
\begin{alignstar}
\psi_n^E &\colon E \to E_n &\text{ corresponds to }&&@{term C_to_CValue_take}@{text "::"}&@{typeof C_to_CValue_take} \\
\psi_n^A &\colon A \to A &\text{ corresponds to }&&@{term CValue_take}@{text "::"}&@{typeof "CValue_take :: nat \<Rightarrow> CValue \<rightarrow> CValue" } \\
