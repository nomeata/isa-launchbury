...
case goal1 thus ?case
  unfolding eqvt_def CESem_graph_aux_def by simp
next
case (goal3 P x)
  show ?case
  proof(cases x)
...
case (goal13 as \<rho> body as' \<rho>' body')
  assume eqvt1: "\<And> e x. e \<in> snd ` set (asToHeap as) \<Longrightarrow> eqvt_at CESem_sumC (e, x)"
    and eqvt2:"eqvt_at CESem_sumC (body, has_ESem.HSem (\<lambda>x0 x1. CESem_sumC (x0, x1)) (asToHeap as) \<rho>)"
    and eqvt3:"\<And>e x. e \<in> snd ` set (asToHeap as') \<Longrightarrow> eqvt_at CESem_sumC (e, x)"
    and eqvt4:"eqvt_at CESem_sumC (body', has_ESem.HSem (\<lambda>x0 x1. CESem_sumC (x0, x1)) (asToHeap as') \<rho>')"
  ...
  have "CESem_sumC (body, has_ESem.HSem (\<lambda>x0 x1. CESem_sumC (x0, x1)) (asToHeap as) \<rho>) =
        CESem_sumC (body', has_ESem.HSem (\<lambda>x0 x1. CESem_sumC (x0, x1)) (asToHeap as') \<rho>)"
  proof (rule Abs_lst_fcb[where ba = bn and f = "(\<lambda> as (body, as'). CESem_sumC (body, has_ESem.HSem (\<lambda>x0 x1. CESem_sumC (x0, x1)) (asToHeap as) \<rho>))", OF tmp pure_fresh pure_fresh, simplified])
    fix p
    ...
    have "(p \<bullet> CESem_sumC) (body', has_ESem.HSem (\<lambda>x xa. (p \<bullet> CESem_sumC) (x, xa)) (asToHeap as') (p \<bullet> \<rho>)) =
          (p \<bullet> CESem_sumC) (body', has_ESem.HSem (\<lambda>x xa. CESem_sumC (x, xa)) (asToHeap as') (p \<bullet> \<rho>))"
      apply (rule arg_cong[OF HSem_cong[OF _ refl refl]])
      apply (subst eqvt_at_apply[OF eqvt3], assumption)
      by rule
    also have "\<dots> = (p \<bullet> CESem_sumC) (body', has_ESem.HSem (\<lambda>x xa. CESem_sumC (x, xa)) (asToHeap as') \<rho>')"
      by (simp add: `p \<bullet> \<rho> = \<rho>` `\<rho>' = \<rho>`)
    also have "\<dots> = CESem_sumC (body', has_ESem.HSem (\<lambda>x xa. CESem_sumC (x, xa)) (asToHeap as') \<rho>')"
      by (rule eqvt_at_apply[OF eqvt4])
    also have "\<dots> = CESem_sumC (body', has_ESem.HSem (\<lambda>x xa. CESem_sumC (x, xa)) (asToHeap as') \<rho>)"
      by (simp add: `\<rho>' = \<rho>`)
    finally
    show "(p \<bullet> CESem_sumC) (body', has_ESem.HSem (\<lambda>x xa. (p \<bullet> CESem_sumC) (x, xa)) (asToHeap as') (p \<bullet> \<rho>)) =
               CESem_sumC (body', has_ESem.HSem (\<lambda>x0 x1. CESem_sumC (x0, x1)) (asToHeap as') \<rho>)".
  qed
