...
locale FutureAnalysis =
 fixes Texp :: "exp \<Rightarrow> Arity \<rightarrow> future set"
begin

fun FBinds :: "heap \<Rightarrow> AEnv \<rightarrow> (var \<Rightarrow> future set)"
  where "FBinds [] = (\<Lambda> ae. \<bottom>)"
      | "FBinds ((x,e)#\<Gamma>) = (\<Lambda> ae. (\<lambda> x'. (if x' = x then fup\<cdot>(Texp e)\<cdot>(ae x) else (FBinds \<Gamma>\<cdot>ae) x)))"

lemma FBinds_Nil_simp[simp]: "FBinds []\<cdot>ae = \<bottom>" by simp

lemma FBinds_Cons[simp]:
  "FBinds ((x,e)#\<Gamma>)\<cdot>ae = (\<lambda> x'. (if x' = x then fup\<cdot>(Texp e)\<cdot>(ae x) else (FBinds \<Gamma>\<cdot>ae) x))" by simp
...
begin
  ...
  next
    ...
    proof
      ...
      hence "\<exists> f \<in> future_add (Texp e\<cdot>a) (Fstack S). f x > 1"
        using future_add_subset2 by auto
      hence "[x,x] \<in> paths (FBinds \<Gamma>\<cdot>ae) (future_add (Texp e\<cdot>a) (Fstack S))" 
        apply -
        apply (erule bexE)
      ...
      hence "pathCard [x,x] x \<sqsubseteq> pathsCard (paths (FBinds \<Gamma>\<cdot>ae) (future_add (Texp e\<cdot>a) (Fstack S))) x"
  ...
  next
    ...
    have "future_add (Texp e\<cdot>(inc\<cdot>a)) (may_call x (Fstack S)) = future_add (may_call x (Texp e\<cdot>(inc\<cdot>a))) (Fstack S)"
      by (rule future_add_may_call_twist)
    hence "(future_add (Texp e\<cdot>(inc\<cdot>a)) (may_call x (Fstack S))) = future_add (Texp (App e x)\<cdot>a) (Fstack S)"
      by (simp add: Texp_App)
    thus "prognosis ae (inc\<cdot>a) (\<Gamma>, e, Arg x # S) \<sqsubseteq> prognosis ae a (\<Gamma>, App e x, S)"
      by simp
  next
    fix ae a \<Gamma> e y x S
    have "Texp e[y::=x]\<cdot>(pred\<cdot>a) = may_call x (Texp (Lam [y]. e)\<cdot>a)" 
      by (simp add: Texp_Lam Texp_subst)
    hence "future_add (Texp e[y::=x]\<cdot>(pred\<cdot>a)) (Fstack S) = future_add (Texp (Lam [y]. e)\<cdot>a) (may_call x (Fstack S))"
  ...
  next
    ...
    have "paths (FBinds (delete x \<Gamma>)\<cdot>ae) (future_add (Texp e\<cdot>u) (Fstack S)) \<subseteq> {tl p |p. p \<in> paths (FBinds \<Gamma>\<cdot>ae) (future_add (Texp (Var x)\<cdot>a) (Fstack S)) \<and> hd p = x}"
    proof
      fix p 
      assume "p \<in> paths (FBinds (delete x \<Gamma>)\<cdot>ae) (future_add (Texp e\<cdot>u) (Fstack S))"

      have "x# p \<in>paths (FBinds \<Gamma>\<cdot>ae) (future_add (Texp (Var x)\<cdot>a) (Fstack S))"
      proof
        
      qed
      thus "p \<in> {tl p |p. p \<in> paths (FBinds \<Gamma>\<cdot>ae) (future_add (Texp (Var x)\<cdot>a) (Fstack S)) \<and> hd p = x}" by force
    qed
    hence "pathsCard (paths (FBinds (delete x \<Gamma>)\<cdot>ae) (future_add (Texp e\<cdot>u) (Fstack S))) \<sqsubseteq> pathsCard {tl p |p. p \<in> paths (FBinds \<Gamma>\<cdot>ae) (future_add (Texp (Var x)\<cdot>a) (Fstack S)) \<and> hd p = x}"
      by (rule pathsCard_mono)
    hence "pathsCard (paths (FBinds (delete x \<Gamma>)\<cdot>ae) (future_add (Texp e\<cdot>u) (Fstack S))) \<sqsubseteq> record_call x\<cdot>(pathsCard (paths (FBinds \<Gamma>\<cdot>ae) (future_add (Texp (Var x)\<cdot>a) (Fstack S))))"
