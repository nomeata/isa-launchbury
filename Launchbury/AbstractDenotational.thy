...
where
 ...
 "ESem (Lam [x]. e) = (\<Lambda> \<rho>. tick $ (Fn $ (\<Lambda> v. ESem e \<cdot> ((\<rho> f|` fv (Lam [x]. e))(x f\<mapsto> v)))))"
...
| "ESem (App e x) = (\<Lambda> \<rho>. tick $ (Fn_project $ (ESem e \<cdot> \<rho>) $ (\<rho> f!\<^sub>\<bottom> x)))"
| "ESem (Var x) = (\<Lambda> \<rho>. tick $ (\<rho> f!\<^sub>\<bottom> x))"
  (* Restrict \<rho> to avoid having to demand set (bn as) \<sharp>* \<rho> *)
| "ESem (Let as body) = (\<Lambda> \<rho>. tick $ (ESem body \<cdot> (has_ESem.HSem ESem (asToHeap as) \<cdot>  (\<rho> f|` fv (Let as body)))))"
proof-
