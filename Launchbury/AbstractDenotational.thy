...
| "\<lbrakk> Let as body \<rbrakk>\<^bsub>\<rho>\<^esub> = tick (\<lbrakk>body\<rbrakk>\<^bsub>has_ESem.UHSem AESem (asToHeap as) (\<rho> f|` fv (Let as body))\<^esub>)"
proof-
txt {* The following proofs discharge technical obligations generated by the Nominal package. *}
...
case (goal13 as body \<rho> as' body' \<rho>')
  assume eqvt1: "\<And> e x. e \<in> snd ` set (asToHeap as) \<Longrightarrow> eqvt_at AESem_sumC (e, x)"
    and eqvt2:"eqvt_at AESem_sumC (body, has_ESem.UHSem (\<lambda>x0 x1. AESem_sumC (x0, x1)) (asToHeap as) (\<rho> f|` fv (Terms.Let as body)))"
    and eqvt3:"\<And>e x. e \<in> snd ` set (asToHeap as') \<Longrightarrow> eqvt_at AESem_sumC (e, x)"
    and eqvt4:"eqvt_at AESem_sumC (body', has_ESem.UHSem (\<lambda>x0 x1. AESem_sumC (x0, x1)) (asToHeap as') (\<rho>' f|` fv (Terms.Let as' body')))"
  ...
  have "AESem_sumC (body, has_ESem.UHSem (\<lambda>x0 x1. AESem_sumC (x0, x1)) (asToHeap as) (\<rho> f|` fv (Let as body)))
      = p \<bullet> AESem_sumC (body, has_ESem.UHSem (\<lambda>x0 x1. AESem_sumC (x0, x1)) (asToHeap as) (\<rho> f|` fv (Let as body)))"
    by (rule permute_pure[symmetric])
  also have "\<dots> = (p \<bullet> AESem_sumC) (body', has_ESem.UHSem (\<lambda>x xa. (p \<bullet> AESem_sumC) (x, xa)) (asToHeap as') ((p \<bullet> \<rho>) f|` fv (Let as' body')))"
    by simp
  also have "\<dots> = (p \<bullet> AESem_sumC) (body', has_ESem.UHSem (\<lambda>x xa. AESem_sumC (x, xa)) (asToHeap as') ((p \<bullet> \<rho>) f|` fv (Let as' body')))"
  ...
  also have "(p \<bullet> AESem_sumC) (body', has_ESem.UHSem (\<lambda>x xa. AESem_sumC (x, xa)) (asToHeap as') (\<rho> f|` fv (Terms.Let as' body')))
    = AESem_sumC (body', has_ESem.UHSem (\<lambda>x xa. AESem_sumC (x, xa)) (asToHeap as') (\<rho> f|` fv (Terms.Let as' body')))"
    by (rule eqvt_at_apply[OF eqvt4, unfolded `\<rho>' = \<rho>`])
  finally show ?case unfolding `\<rho>' = \<rho>` by simp
qed auto
