...
nominal_primrec
  AESem :: "exp \<Rightarrow> (var f\<rightharpoonup> 'Value) \<rightarrow> 'Value"
where
  (* Restrict \<rho> to avoid having to demand atom x \<sharp> \<rho> *)
 "AESem (Lam [x]. e) = (\<Lambda> \<rho>. tick $ (Fn $ (\<Lambda> v. AESem e \<cdot> ((\<rho> f|` fv (Lam [x]. e))(x f\<mapsto> v)))))"
...
| "AESem (App e x) = (\<Lambda> \<rho>. tick $ (Fn_project $ (AESem e \<cdot> \<rho>) $ (\<rho> f!\<^sub>\<bottom> x)))"
| "AESem (Var x) = (\<Lambda> \<rho>. tick $ (\<rho> f!\<^sub>\<bottom> x))"
  (* Restrict \<rho> to avoid having to demand set (bn as) \<sharp>* \<rho> *)
| "AESem (Let as body) = (\<Lambda> \<rho>. tick $ (AESem body \<cdot> (has_ESem.UHSem AESem (asToHeap as) \<cdot>  (\<rho> f|` fv (Let as body)))))"
proof-
txt {* The following proofs discharge technical obligations generated by the Nominal package. *}

case goal1 thus ?case
  using [[show_sorts]]
  unfolding eqvt_def AESem_graph_aux_def
  apply rule
  apply (perm_simp)
...
case (goal4 x e x' e')
  ...
  proof (elim conjE disjE)
    ...
    { fix xa \<rho>
      have "AESem_sumC e \<cdot> ((\<rho> f|` fv (Lam [x]. e))(x f\<mapsto> xa)) = (x' \<leftrightarrow> x) \<bullet> AESem_sumC e \<cdot> ((\<rho> f|` fv (Lam [x]. e))(x f\<mapsto> xa))" by (simp add: permute_pure)
      also have "\<dots> = ((x' \<leftrightarrow> x) \<bullet> AESem_sumC) e' \<cdot> ((((x' \<leftrightarrow> x) \<bullet> \<rho>) f|` (fv e' - {x'}))(x' f\<mapsto> xa))" by simp
      also have "((x' \<leftrightarrow> x) \<bullet> AESem_sumC) e' = AESem_sumC e'"
      ...
      have "AESem_sumC e\<cdot>((\<rho> f|` fv (Lam [x]. e))(x f\<mapsto> xa)) = AESem_sumC e'\<cdot>((\<rho> f|` fv (Lam [x']. e'))(x' f\<mapsto> xa))".
    }
    thus ?thesis by simp
  qed simp
next
...
case (goal13 as body as' body')
  assume eqvt1: "\<And> x e. e \<in> snd ` set (asToHeap as) \<Longrightarrow> eqvt_at AESem_sumC e"
    and eqvt2: "\<And> x. eqvt_at AESem_sumC body"
    and eqvt3:"\<And> x e. e \<in> snd ` set (asToHeap as') \<Longrightarrow> eqvt_at AESem_sumC e"
    and eqvt4:"\<And> x. eqvt_at AESem_sumC body'"
  ...
  { fix \<rho>
  have "AESem_sumC body \<cdot> (has_ESem.UHSem AESem_sumC (asToHeap as) \<cdot> (\<rho> f|` fv (Let as body)))
      = p \<bullet> AESem_sumC body \<cdot> (has_ESem.UHSem AESem_sumC (asToHeap as) \<cdot> (\<rho> f|` fv (Let as body)))"
    by (rule permute_pure[symmetric])
  also have "\<dots> = (p \<bullet> AESem_sumC) body' \<cdot> (has_ESem.UHSem (p \<bullet> AESem_sumC) (asToHeap as') \<cdot>  ((p \<bullet> \<rho>) f|` fv (Let as' body')))"
    by simp
  also have "has_ESem.UHSem (p \<bullet> AESem_sumC) (asToHeap as') = has_ESem.UHSem AESem_sumC (asToHeap as')"
    by (rule UHSem_cong[OF eqvt_at_apply[OF eqvt3] refl])
  also have "(p \<bullet> \<rho>) f|` fv (Let as' body') = \<rho> f|`  fv (Let as' body')"
    by (rule fmap_restr_perm[OF *], simp)
  also have "(p \<bullet> AESem_sumC) body' = AESem_sumC body'"
    by (rule eqvt_at_apply[OF eqvt4])
  finally
  have "AESem_sumC body\<cdot>(has_ESem.UHSem AESem_sumC (asToHeap as)\<cdot>(\<rho> f|` fv (Terms.Let as body))) =
        AESem_sumC body'\<cdot>(has_ESem.UHSem AESem_sumC (asToHeap as')\<cdot>(\<rho> f|` fv (Terms.Let as' body')))".
  }
  thus ?case  by simp
qed auto
termination (in semantic_domain) (eqvt) by lexicographic_order

sublocale has_ESem AESem.

abbreviation AESem_syn' ("\<lbrakk>_\<rbrakk>\<^bsub>_\<^esub>"  [60,60] 60) where "\<lbrakk>e\<rbrakk>\<^bsub>\<rho>\<^esub> \<equiv> AESem e \<cdot> \<rho>"
abbreviation AHSem_syn ("\<lbrace>_\<rbrace>_"  [60,60] 60) where "\<lbrace>\<Gamma>\<rbrace>\<rho> \<equiv> UHSem \<Gamma> \<cdot> \<rho>"
