...
| "AESem (Let as body) = (\<Lambda> \<rho>. tick $ (AESem body \<cdot> (has_ESem.HSem AESem (asToHeap as) \<cdot>  (\<rho> f|` fv (Let as body)))))"
proof-
...
case (goal13 as body as' body')
  ...
  { fix \<rho>
  have "AESem_sumC body \<cdot> (has_ESem.HSem AESem_sumC (asToHeap as) \<cdot> (\<rho> f|` fv (Let as body)))
      = p \<bullet> AESem_sumC body \<cdot> (has_ESem.HSem AESem_sumC (asToHeap as) \<cdot> (\<rho> f|` fv (Let as body)))"
    by (rule permute_pure[symmetric])
  also have "\<dots> = (p \<bullet> AESem_sumC) body' \<cdot> (has_ESem.HSem (p \<bullet> AESem_sumC) (asToHeap as') \<cdot>  ((p \<bullet> \<rho>) f|` fv (Let as' body')))"
    by simp
  also have "has_ESem.HSem (p \<bullet> AESem_sumC) (asToHeap as') = has_ESem.HSem AESem_sumC (asToHeap as')"
  ...
  have "AESem_sumC body\<cdot>(has_ESem.HSem AESem_sumC (asToHeap as)\<cdot>(\<rho> f|` fv (Terms.Let as body))) =
        AESem_sumC body'\<cdot>(has_ESem.HSem AESem_sumC (asToHeap as')\<cdot>(\<rho> f|` fv (Terms.Let as' body')))".
  }
  thus ?case  by simp
qed auto
...
lemmas HSem_eqvt' = HSem_eqvt[of _ AESem, unfolded permute_ESem]
