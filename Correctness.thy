...
proof-
  ...
  have "\<lbrace>\<Gamma>\<rbrace> = fmap_restr (fst ` set \<Gamma>) (\<lbrace>(x, e) # \<Gamma>\<rbrace>)"
    apply (rule HSem_add_fresh[OF fempty_is_heapExtendJoin_cond'_ESem fempty_is_heapExtendJoin_cond'_ESem, simplified (no_asm), unfolded heapVars_def, symmetric])
  ...
  also have "... = \<lbrace>\<Delta>\<rbrace>"
    apply (rule HSem_add_fresh[OF fempty_is_heapExtendJoin_cond'_ESem fempty_is_heapExtendJoin_cond'_ESem, simplified (no_asm), unfolded heapVars_def])
    using fresh apply (simp add: fresh_Pair)
    done
  ...
  have "\<lbrakk>e\<rbrakk>\<^bsub>\<lbrace>\<Gamma>\<rbrace>\<^esub> = \<lbrakk>e\<rbrakk>\<^bsub>\<lbrace>(x, e) # \<Gamma>\<rbrace>\<^esub>"
    apply (rule ESem_add_fresh[OF fempty_is_heapExtendJoin_cond'_ESem fempty_is_heapExtendJoin_cond'_ESem, symmetric])
  ...
  also have "... = \<lbrakk>z\<rbrakk>\<^bsub>\<lbrace>(x, z) # \<Delta>\<rbrace>\<^esub>"
    apply (rule the_lookup_HSem_heap[of _ "(x, z) # \<Delta>" x, OF fempty_is_heapExtendJoin_cond'_ESem, simplified (no_asm)])
    apply simp_all    
    done
  also have "... =  \<lbrakk>z\<rbrakk>\<^bsub>\<lbrace>\<Delta>\<rbrace>\<^esub>"
    apply (rule ESem_add_fresh[OF fempty_is_heapExtendJoin_cond'_ESem fempty_is_heapExtendJoin_cond'_ESem])
    using fresh by (simp add: fresh_Pair)
