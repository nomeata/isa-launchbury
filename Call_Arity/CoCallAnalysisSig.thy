...
begin
  abbreviation ccExp_syn ("\<G>\<^bsub>_\<^esub>") where "\<G>\<^bsub>a\<^esub> \<equiv> (\<lambda>e. ccExp e\<cdot>a)"
  abbreviation ccExp_bot_syn ("\<G>\<^sup>\<bottom>\<^bsub>_\<^esub>") where "\<G>\<^sup>\<bottom>\<^bsub>a\<^esub> \<equiv> (\<lambda>e. fup\<cdot>(ccExp e)\<cdot>a)"
end
