...
next
  ...
  next
    case False with `inc\<cdot>a \<sqsubseteq> a'` have "a \<sqsubseteq> a'" 
...
proof (rule eq\<rho>I)
  ...
  with `ae \<sqsubseteq> ae'` have "up\<cdot>a \<sqsubseteq> ae' x" by (metis fun_belowD)
  then obtain a' where "ae' x = up\<cdot>a'" by (metis Exh_Up below_antisym minimal)
  with `eq\<rho> ae' \<rho>1 \<rho>2`
  have "eq a' (\<rho>1 x) (\<rho>2 x)" by (auto simp add: eq\<rho>.simps)
  with `up\<cdot>a \<sqsubseteq> ae' x` and `ae' x = up\<cdot>a'`
...
proof(induction a e arbitrary: \<rho>1 \<rho>2 rule: transform.induct[case_names App Lam Var Let Bool IfThenElse])
  case (Var a x)
  from `eq\<rho> (Aexp (Var x)\<cdot>a) \<rho>1 \<rho>2` 
